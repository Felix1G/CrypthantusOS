...
typedef enum
{
    ...
    GDT_ACCESS_CODE_SEG = 0x18,
    GDT_ACCESS_DATA_SEG = 0x10,
...
#define GDT_FLAG_LIMIT_HI(limit, flags) (((limit >> 16) & 0xF) | (flags & 0xF0))
#define GDT_BASE_HIGH(base) ((base >> 24) & 0xFF)
#define GDT_ENTRY_MACRO(base, limit, access, flags) {     \
    GDT_LIMIT_LOW(limit),                           \
    GDT_BASE_LOW(base),                             \
    GDT_BASE_MID(base),                             \
    access,                                         \
    GDT_FLAG_LIMIT_HI(limit, flags),                \
    GDT_BASE_HIGH(base),                            \
}

GDT_ENTRY gdt_entry[] = {
    GDT_ENTRY_MACRO(0, 0, 0, 0), //null descriptor
    GDT_ENTRY_MACRO(0, 0xFFFFF,
                    GDT_ACCESS_PRESENT | GDT_ACCESS_RING0 | 
                    GDT_ACCESS_CODE_SEG | GDT_ACCESS_CODE_READ,
                    GDT_FLAG_32_BIT | GDT_FLAG_GRANULARITY_4K),
    GDT_ENTRY_MACRO(0, 0xFFFFF,
                    GDT_ACCESS_PRESENT | GDT_ACCESS_RING0 | 
                    GDT_ACCESS_DATA_SEG | GDT_ACCESS_DATA_WRITE,
                    GDT_FLAG_32_BIT | GDT_FLAG_GRANULARITY_4K),
};
